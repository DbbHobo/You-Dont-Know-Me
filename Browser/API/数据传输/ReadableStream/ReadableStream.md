...
Stream API 中的 `ReadableStream` 接口表示**可读的字节数据流**。在浏览器中，最典型的 `ReadableStream` 就是 `fetch` 请求的响应体。`Fetch API` 通过 `Response.body` 提供了一个具体的 `ReadableStream` 对象。

- 分块读取：数据是一块一块（Chunks）到达的。
- 背压支持：如果下游处理太慢，它可以告诉上游暂停发送。
