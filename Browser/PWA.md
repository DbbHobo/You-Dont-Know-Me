...
它是一个简单的 `JSON` 文件，告诉浏览器关于 Web 应用的元数据。让网站可以被“安装”到系统桌面，并摆脱浏览器的地址栏、工具栏。

- `icons`: 桌面图标长什么样。
- `display`: 是否显示地址栏（standalone 会隐藏地址栏，看起来像真 App）。
- `theme_color`: 手机状态栏的颜色。
- `start_url`: 从桌面点开时，默认进入哪个页面。
