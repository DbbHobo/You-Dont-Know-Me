...
为什么有 `_currentValue` / `_currentValue2` 两个值？

- 在大多数 Web 开发场景下，我们只使用 `react-dom`，此时只有 `_currentValue` 在起作用。
- 但在某些复杂的混合场景中，可能会有两个 React 渲染器同时运行：场景 A (移动端): `React Native` (主渲染器) 和 `Fabric` (新版渲染器) 同时存在；场景 B (桌面端/Web): `React DOM` (主渲染器) 和 `React ART` (次渲染器，用于绘图) 同时存在。
...
1. 首先调用`pushProvider()`方法更新值，并将最新值和`fiber`入栈`valueStack`、`fiberStack`；
...
在`provider`的`completeWork`工作之前，其实会先处理子节点的`beginWork`，因此，子节点`consumer`在`beginWork`阶段就消费了`context`内容，子节点`consumer`会将依赖的`context`存储在`dependencies`属性中，要么通过`useContext hook`要么通过`JSX`语法。
...
#### Consumer 通过 hook 方式或 JSX 方式消费(`completeWork`)

`Consumer`组件在`completeWork`阶段仍然走`ContextConsumer`分支，如果有`context`值的更新会给使用了`context`值的节点打上更新标记`flags`，最后调用`bubbleProperties(workInProgress)`将`flags`冒泡往上，其他并未操作。最后在`commit`阶段更新视图。
...
2. `beginWork`阶段首先遇到`REACT_PROVIDER_TYPE`类型的`Provider`组件，调用`updateContextProvider`=>`pushProvider()`，将最新值和当前`fiber`入栈；
