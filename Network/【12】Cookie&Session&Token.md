...
- 使用 **`httpOnly`** 和 **`Secure`** 在一定程度上提高安全性，这样 JavaScript 脚本无法读取，极大降低了被盗风险
...
使用`token`进行身份验证的标准工作流

1. 登录：用户提交凭据（用户名/密码）。
2. 颁发：服务器验证成功后，同时生成 `Access Token`(过期时间设为 15 分钟) 和 `Refresh Token`(过期时间设为 7 天) 返回给客户端。
3. 使用：客户端将 `Access Token` 放入 `HTTP` 请求头（`Authorization: Bearer <token>`）中访问受保护资源。
4. 过期处理：当 `Access Token` 过期时，服务器返回 `401 Unauthorized` 错误。
5. 刷新：客户端检测到 401 后，自动调用 `/refresh` 接口，发送 `Refresh Token`。服务器验证 `Refresh Token` 合法后，返回新的 `Access Token`。
...
[Access Token and Refresh Token](https://dev.to/zeeshanahmad9/access-token-and-refresh-token-2i2h?context=digest)
