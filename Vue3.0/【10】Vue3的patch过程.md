...
`createVNode`生成 VNode，`render(vnode, rootContainer, isSVG)`将`VNode`渲染成真实`DOM`：
...
当`VNode`存在时，就会进入`patch`方法，以上是根组件 app 实例的生成过程，然后会深入进行内部内容的`patch`生成。

## Patch 关键参数

`VNode`节点中，有几个用于`patch`方法的关键字段如下：
...
- `patchFlags`：描述节点可能“动态”的类型，可能是动态文本/动态Class绑定/带有key的序列等等。

`patchFlags` 用于标识`VNode`应该如何`patch`的类型，在编译的`tansform`优化阶段生成，用于运行时优化。通过进行 `｜` 或运算进行标记的组合，如果当前节点是一个动态文本节点(0000 0001)，它同时又具有动态 style (0000 0100)，二者进行 `｜` 或运算后值为 (0000 0101)。
...
- `shapeFlags`：描述了节点及其子节点的特征组合。

`shapeFlags` 描述了节点及其子节点的特征组合，在生成`VNode`阶段生成，便于在`patch`阶段，根据不同的组件类型执行相应的逻辑。
...
可以看到`patch`方法由`baseCreateRenderer`生成并返回，`patch`方法根据`n2`也就是新`VNode`的`type`进入不同的处理分支：`processText`/`processCommentNode`/`processFragment`/`processElement`/`processComponent`等等。这些分支方法都以闭包的形式被调用，并不暴露给外部。
...
function baseCreateRenderer(
  ...
  const render: RootRenderFunction = (vnode, container, namespace) => {
    ...
    if (vnode == null) {
      ...
      // 【app实例的mount方法会调用render方法，从而进入patch】
...
- 提取`const { type, ref, shapeFlag } = n2`，然后根据`type`的不同`switch`到不同分支。
  ...
  - `type`是`Static`静态节点
  - `type`是`Fragment`空节点
  - 此外的节点继续根据`ShapeFlags`判断是`ELEMENT`、`COMPONENT`、`TELEPORT`或者`SUSPENSE`类型的组件进行处理
...
`n1.type === n2.type && n1.key === n2.key`判断才认为新旧`VNode`是相同类型的 VNode
...
- `case Static`: `mountStaticNode(n2, container, anchor, isSVG)`/`patchStaticNode(n1, n2, container, isSVG)`
...
- `default`
  ...
  - `FEATURE_SUSPENSE && shapeFlag & ShapeFlags.SUSPENSE`

可以看到这些分支处理不同类型的节点都会根据旧节点`n1`是否存在去`mount`或`patch`。

`patch`方法在调用的时候是一个“深度优先遍历”的过程，最终会走到单个`Element`的处理方法上插入父节点直到回到`app`实例对应`VNode`。对于`Element`/`Fragment`/`Component`等类型的节点都可能会继续`patch`子节点，根据子节点不同的情况进入不同的处理方法。

### `processElement`

- `mountElement`: 1.创建节点对应`DOM`；2.有子节点去`patch`子节点；3.`DOM`插入父容器；
- `patchElement`: 1.复用旧节点对应`DOM`；2.有子节点去`patch`子节点；3.更新`DOM`；
...
const patchElement = (
  ...
  // 【patchFlag>0，需要根据patchFlag进行不同的props的对比】
  if (patchFlag > 0) {
...
- `mountComponent`: 1.调用 **`createComponentInstance`** 创建组件实例；2.调用 **`setupComponent`** 确定组件的`render`函数；3.调用 **`setupRenderEffect`** 设置组件渲染逻辑并调用`instance.update()` => `effect.run()` => `componentUpdateFn()`
- `updateComponent`: 1.调用组件实例`instance.update()` => `effect.run()` => `componentUpdateFn()`
...
- 一个`Block`其实就是一个块，一块 `VNode`，在编译期间我们会拿到所有的动态节点，它们存储在 `dynamicChildren` 中，因此在 `diff` 过程中就可以避免按照 `VNode` 树一层一层的遍历，而是直接找到 `dynamicChildren` 进行更新
...
const processFragment = (
  ...
  if (n1 == null) {
    // 【第一个分支：旧VNode不存在，初次挂载阶段，调用mountChildren】
...
`mountChildren` 方法是在旧节点不存在的情况下直接对新节点进行生成挂载，也就是首次挂载。

`patchBlockChildren`方法是在`STABLE_FRAGMENT`情况下，根据新旧**动态子节点 dynamicChildren**进行不同的对比操作，最后进入`patch`递归。

`patchChildren`根据是否有`key`可以进入`patchKeyedChildren`或`patchUnkeyedChildren`分支，在新旧子节点都是多节点时就到了我们常说的**diff 核心**算法。
...
当存在`dynamicChildren`时且当前`block`的`patchFlag`是`STABLE_FRAGMENT`调用，相当于直接对比`dynamicChildren`节点即可，继续执行`patch`方法进入更深层次的解析：
...
`patchChildren`方法可能分支有：

由 Vue 编译器生成的带有`patchFlag`的情况：

- `patchKeyedChildren`
- `patchUnkeyedChildren`

否则，根据新旧节点的`shapeFlag`走不通分支的情况：

1. 新节点是文本：先`unmountChildren`旧节点然后调用`hostSetElementText`插入文本

2. 新节点是数组
   1. 旧节点也是数组：调用`patchKeyedChildren`进行diff
   2. 旧节点是文本或者空：调用`unmountChildren`卸载旧节点，`mountChildren`创建并插入新节点

3. 新节点是空：
   1. 旧节点是数组：调用`unmountChildren`销毁旧节点
   2. 旧节点是文本或者空：调用`hostSetElementText`清空内容
...
5. 包含新增、删除、移动等多种情况的序列
   1. 首先为新节点建一个`keyToNewIndexMap`的 Map 用于存储【新节点 key，新节点的索引】这样的一个结构
   2. 新建一个`newIndexToOldIndexMap`的 Array 长度等于剩余需要`patch`节点个数(去掉可复用的头尾部)，找到相同 key 的新节点然后在`newIndexToOldIndexMap`中新节点所在索引位置存储【旧节点（在新节点能找到复用的）的旧索引+1】，并且判断旧节点序列是否需要移动操作，如果索引是完全递增的代表不需要移动，否则就是需要
   3. 找出`newIndexToOldIndexMap`中的最长递增子序列，这个序列是所有不需要移动的节点，剩下的节点就是需要移动的或者新增的，然后进行移动和新增操作
...
Vue2 中的 diff 算法采取的是**同层比较**的方式，是 full diff。

Vue3 中的 diff 算法中有`block`、`fragment`和`patchFlags`等概念，优化了 diff 过程，去掉了很多不必要的 diff 过程。`patchFlags`就是用于进行判断是否静态节点，是否动态文本，是否动态属性等等判断条件，所以带有 `patchFlags` 的节点就是动态节点，我们将它提取出来放到一个`dynamicChildren`数组中存储。拿到了所有的动态节点，它们存储在 `dynamicChildren`数组中，因此在 `diff` 过程中就可以避免按照虚拟 VNode 树一层一层的遍历，而是直接找到 `dynamicChildren` 进行更新。

还有一个优化点在于提升静态节点，或静态的属性，这可以减少创建 VNode 的消耗，静态提升是以树为单位的。

然后对于多个动态有 key 节点的 diff 过程，通过寻找最长子序列算法尽可能少的复用和减少节点的移动也是一个性能提升点。

## 总结

![patch](./assets/patch.svg)
