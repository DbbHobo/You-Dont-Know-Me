...
前文我们讲到，`const app = ensureRenderer().createApp(...args)`会创建 app 实例，app 实例再调用`mount`方法，`mount`方法中关键的步骤如下，这是组件构造的入口：
...
创建子组件的`VNode`由`renderComponentRoot`生成，本质上是去调用组件实例的`instance.render`方法，这个方法由 `template` 编译生成或者 `setup` 函数返回。
...
官方示例 tree.html 中我们可以看到生成的组件 `VNode` 如下：
...
处理根 App 组件的路径如下，从 App 开始往下依次处理子组件：
...
组件的**首次挂载**调用`mountComponent`方法又分为几个步骤：

`mountComponent` => `createComponentInstance` + `setupComponent` + `setupRenderEffect`
...
![vue](./assets/component/createComponentInstance1.png)
![vue](./assets/component/createComponentInstance2.png)
...
![vue](./assets/component/setupComponent1.png)
![vue](./assets/component/setupComponent2.png)
![vue](./assets/component/setupComponent3.png)
![vue](./assets/component/setupComponent4.png)

3. 【第三步】：调用 **`setupRenderEffect`** 设置组件渲染逻辑并调用
   - 构造`componentUpdateFn`方法，这期间`renderComponentRoot`生成组件内容的 VNode，`patch`循环遍历进行 DOM 挂载，这两个方法执行之前会执行`beforeMount`/`beforeUpdate`生命周期钩子，这两个方法执行完成之后会执行`mounted`/`updated`生命周期钩子
   - 调用`new ReactiveEffect`生成本组件对应的`render effect`，`componentUpdateFn`方法作为回调函数传入，响应式数据变化就会引起该`render effect`调用`run`方法继而调用`componentUpdateFn`方法，`componentUpdateFn`方法会根据实例上的`isMounted`是否为 true，进入首次挂载分支，否则进入更新分支
   - 定义`update`方法为`effect.run()`并调用`update`方法，继而调用`componentUpdateFn`回调，进入**首次挂载分支**
   - `componentUpdateFn`回调会调用`renderComponentRoot(instance)`生成组件内容的`VNode`，继而调用`patch`方法进行新旧`VNode`对比和 DOM 挂载
...
![vue](./assets/component/setupRenderEffect1.png)
![vue](./assets/component/setupRenderEffect2.png)
![vue](./assets/component/componentUpdateFn1.png)
![vue](./assets/component/componentUpdateFn2.png)
![vue](./assets/component/componentUpdateFn3.png)

可以看到`renderComponentRoot(instance)`提取了组件实例`instance`上相关内容，然后调用`render`方法构造了组件的`VNode`：
...
export function renderComponentRoot(instance: ComponentInternalInstance): VNode {
  ...
  try {
    // 【STATEFUL_COMPONENT调用render构造VNode】
    if (vnode.shapeFlag & ShapeFlags.STATEFUL_COMPONENT) {
      // withProxy is a proxy with a different `has` trap only for
      // runtime-compiled render functions using `with` block.
      const proxyToUse = withProxy || proxy
      ...
      // 【非STATEFUL_COMPONENT调用render构造VNode】
      // functional
      const render = Component as FunctionalComponent
      // in dev, mark attrs accessed if optional props (attrs === props)
      if (__DEV__ && attrs === props) {
...
然后在`patch`方法中，`mountElement`遇到`DOM`元素会生成对应的`DOM`元素挂载在`VNode`的`el`属性上，深入执行到最底层的 DOM 元素再插入到父 DOM 元素中，逐渐形成完整的 DOM 树。
...
- 核心是执行`instance.update()`方法也就是对应`render effect`的`run()`方法继而调用`componentUpdateFn()`方法，进入**更新分支**
...
在`componentUpdateFn`中，最关键的两步仍旧是：1.`const nextTree = renderComponentRoot(instance)`；2.`patch`，但是在状态更新后，`render`函数中引用的状态变化了，从而引起`VNode`的变化，然后下一步就是对比新旧节点再去进行挂载了。

![vue](./assets/component/componentUpdateFn4.png)
![vue](./assets/component/componentUpdateFn5.png)
...
export function defineAsyncComponent<T extends Component = { new (): ComponentPublicInstance }>(
  ...
  // 【缓存机制，一旦组件加载成功，就放入resolvedComp中，后续直接读取缓存】
...
`createAppAPI`方法生成的 app 实例有`component`方法用于注册全局组件如下：
...
方式一：编译器检测到你在顶层 `import` 了 `Child`，它会直接在生成的 `render` 函数中使用这个变量。

```ts
// 编译后
import Child from "./Child.vue"
function render(_ctx) {
  return (_openBlock(), _createBlock(Child)) // 直接引用变量
}
```

方式二：使用 `export default { components: { Child } }`，编译器不知道 `Child` 是什么，它会在编译父组件时调用 `resolveComponent` 解析这个局部组件的名称，并加入父组件的`instance.components`。
...
5. `componentUpdateFn`方法根据是否首次渲染进入不同分支，核心都是两个步骤，第一步调用`renderComponentRoot(instance)`生成组件内容的`VNode`，第二步调用`patch`方法进行`VNode`对比和 DOM 挂载；

![vue](./assets/component/component.svg)
