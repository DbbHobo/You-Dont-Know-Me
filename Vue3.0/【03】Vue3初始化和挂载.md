...
export const createApp = ((...args) => {
  //【ensureRenderer返回对象包含render和createApp方法，调用该对象的createApp方法生成app实例】
...
然后我们接着去看 `mount` 方法，在 `mount` 方法中会调用 `createVNode` 创建根实例app的 `VNode`， `VNode` 传入 `render` 方法进行渲染解析挂载：
...
首先为根容器生成`VNode`，`patchFlag`为 0(默认值，需要全量diff)，`shapeFlag`为 4(`STATEFUL_COMPONENT`)：
...
可以看到最后返回的是 `createBaseVNode` 如下，`Vue3`构造`VNode` 和`Vue2`并不太一样，增加了 `type`、`patchFlag`、`shapeFlag` 等字段用于后续的`patch`方法优化，同样也会规范化子节点，然后我们可以在源码中看到这样一段注释 presence of a patch flag indicates this node needs patching on updates，说明`patchFlag`用于判断这个 `VNode` 在后续的 `update` 过程中是否需要 `patch` 或者如何去 `patch`，这是`Vue3`在`Vue2`基础上的一个优化点，这是我们首次渲染根实例app，`patchFlag` 使用的是默认值 0，`shapeFlag` 是 `ShapeFlags.STATEFUL_COMPONENT`：
...
这一步通过最终调用`createBaseVNode`方法返回根实例app对应的`VNode`，以供后续挂载使用。
...
从我们的用例来看，先根据`app`实例的`VNode`创建组件实例/获取组件`render`方法/生成组件对应`reactiveEffect`，然后调用组件实例的`render`方法会生成组件的内部内容对应的`VNode`结构(`subtree`)，那么下一步就是构建内部内容。以用例来看，这是一个简单的组件，内部就是一些`DOM`节点，所以`patch`会继续深入处理分支为`ShapeFlags.ELEMENT`，在`processElement=>mountElement`(首次渲染)，`mountElement`中会首先生成该节点对应`DOM`节点，然后看是否有子节点，如果有子节点继续深入`patch`，如果没有就回到上一层，这个时候就会把`DOM`节点插入上层`DOM`节点，根据这个逻辑一直插入到根`container`容器为止，逐渐渲染出页面。

## 总结

![vue](./assets/mount/mount.svg)
