...
首先来看 `createApp` 方法，其中的关键在`ensureRenderer().createApp(...args)`，传入 `options` 会创建一个APP实例并调用 `mount` 方法，`app` 实例包含 `use`、`mixin`、`component`、`directive`、`mount`、`unmount`、`provide` 等方法，`mount` 方法的第一个参数作为根元素：
...
然后我们接着去看 `mount` 方法，在 `mount` 方法会调用 `createVNode` 创建 `VNode`， `VNode` 传入 `render` 方法进行渲染解析挂载：
...
首先为根容器生成`VNode`，`patchFlag`为0，`shapeFlag`为4：
...
然后根据前文我们知道当前`app`实例对应`VNode`的`shapeFlag`是4也就是`STATEFUL_COMPONENT`。当前挂载根组件我们走进的是`processComponent`方法，这个分支，由于是首次渲染所以`n1`为null，所以直接进入`mountComponent`方法，最终会进一步解析包裹的内容生成DOM并渲染：
...
`patch`的过程是由上到下递归的过程，由`createApp()`生成的`app`实例本身可以理解为“根组件”，其余内容都包裹其中，`app`实例会包裹有开发人员写的子组件，一步步通过`patch`解析生成`VNode`并生成`DOM`去渲染。
