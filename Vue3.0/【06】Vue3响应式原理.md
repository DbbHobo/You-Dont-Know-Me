...
export function triggerEffects(
  ...
  for (const effect of dep.keys()) {
    ...
    if (effect._dirtyLevel < dirtyLevel && (tracking ??= dep.get(effect) === effect._trackId)) {
      effect._shouldSchedule ||= effect._dirtyLevel === DirtyLevels.NotDirty
      effect._dirtyLevel = dirtyLevel
    }

    if (effect._shouldSchedule && (tracking ??= dep.get(effect) === effect._trackId)) {
      if (__DEV__) {
        // eslint-disable-next-line no-restricted-syntax
        effect.onTrigger?.(extend({ effect }, debuggerEventExtraInfo))
      }
...
track(debugInfo?: DebuggerEventExtraInfo): Link | undefined {
    ...
    if (link === undefined || link.sub !== activeSub) {
      // 【创建当前dep和sub(reactiveEffect)的连结节点link】
      ...
      if (!activeSub.deps) {
        ...
        // 【当前sub的最后一个dep给link.prevDep，也就是说可以追溯上一个和当前sub有连结关系的dep】
        // 【-----link可以通过prevDep追溯上一个数据源-----】
        link.prevDep = activeSub.depsTail
        // 【当前link赋值给当前sub的最后一个dep，也就是说上一个和当前sub有连结关系的dep的下一个就是当前dep】
        // 【-----link可以通过nextDep追溯下一个数据源-----】
...
function addSub(link: Link) {
  ...
  if (link.sub.flags & EffectFlags.TRACKING) {
    ...
    // 【如果当前dep的订阅者末尾不是当前连结link】
    if (currentTail !== link) {
      // 【当前连结的prevSub设置为上一个当前dep的订阅者】
      link.prevSub = currentTail
      // 【上一个当前dep的订阅者的下一个设置为当前连结link】
      // 【-----link可以通过nextSub追溯下一个订阅者-----】
      if (currentTail) currentTail.nextSub = link
    }

    if (__DEV__ && link.dep.subsHead === undefined) {
      // 【当前dep的订阅者头部设置为当前连结link】
      link.dep.subsHead = link
    }
    // 【当前dep的订阅者队头改成当前连结link】
    // 【-----link可以通过prevSub追溯上一个订阅者-----】
    link.dep.subs = link
  }
}
```
...
![vue](./assets/reactive/reactive.svg)
