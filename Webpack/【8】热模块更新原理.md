...
- `webpack-dev-server` 负责启动本地服务器`Webpack Dev Server`以及和浏览器建立`websocket`通道
- `webpack-dev-middleware` 负责文件相关的操作
- `HotModuleReplacementPlugin` 负责生成补丁文件并将内容注入打包后的文件中
- `webpack/hot/dev-server.js` & `webpack-dev-server/client/index.js` 注入浏览器端，同本地服务器进行通信，感知编译完成并请求补丁文件
...
1. 获取`compiler`实例
2. 使用`express`启动本地`Webpack Dev Server`，让浏览器可以请求本地的静态资源
3. 本地`Webpack Dev Server`启动之后，再去启动`websocket`服务
4. 创建 `webpackDevMiddleware` 中间件实例用于将编译后的文件打包到内存
5. 每次编译完成之后（监听`compiler`实例的`done`事件）本地`Webpack Dev Server`会调用`_sendStats`方法传达信息例如 OK 给浏览器
...
- `webpack-dev-server/client/index.js`：打包进bundle的js文件，浏览器端的通信中枢，负责与 `Webpack Dev Server`（开发服务器）建立连接，接收服务器推送的热更新事件（如文件变化、模块更新）。

1. 浏览器接收到`Websocket`传送的 OK 信息会去调用`reloadApp`方法
2. `reloadApp`方法中会`hotEmitter.emit("webpackHotUpdate",xxx)`**触发**`webpackHotUpdate`事件，表示检测到更新需要`webpack-dev-server`服务器端将更新的内容传过来（`[hash].hot-update.json` 和 `[hash].hot-update.js`）
...
- `webpack/hot/dev-server.js`：打包进bundle的js文件，浏览器端的更新决策者，接收到命令后，决定是尝试“热更新（HMR）”还是无奈“全页刷新（Reload）”。
...
1. 通过`webpack-dev-server`插件创建两个服务器：提供静态资源的本地服务器 `Webpack Dev Server` 服务和 `WebSocket` 服务，并且使用 `webpack-dev-middleware` 中间件监听本地文件的变化
2. `Webpack Dev Server` 负责直接提供静态资源的本地服务器（打包后的资源直接被浏览器请求和解析）
3. `WebSocket` 提供一个联通客户端和`Webpack Dev Server`的长连接，保持客户端和本地服务器双方可以通信
4. `HotModuleReplacementPlugin`它在打包时偷偷给每个模块加上 `module.hot` 接口，并负责生成补丁文件`.json`文件（`manifest`）和`.js`文件（`update chunk`）
5. 文件修改 → Webpack 重新编译 → `webpack-dev-server/client/index.js` 接受到 OK 信息 → 调用`reloadApp`方法 → **触发**`webpackHotUpdate`事件
6. `webpack/hot/dev-server.js`**监听**`webpackHotUpdate`事件调用`check()`方法 → `module.hot.check(true)`过程中调用`HotModuleReplacementPlugin`模块提供的方法去替换旧模块 → 页面内容刷新

![webpack](./assets/HMR.svg)
