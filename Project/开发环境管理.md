...
# 查看 HTTP 响应头非常适合检查状态码或缓存设置）。：
curl -I https://www.google.com（

# 发送 POST 请求：
curl -X POST -d "name=gemini&age=1" https://api.example.com/user

# 保存文件：
curl -O https://example.com/file.zip（使用原文件名保存）。
...
Homebrew installs the stuff you need that Apple (or your Linux system) didn’t. Homebrew installs packages to their own directory and then symlinks their files into /opt/homebrew (on Apple Silicon).

`homebrew` 是 macOS（或 Linux）的“软件管家”。它管理的是系统级的应用程序和命令行工具。比如，你通过它安装 Git、Node.js、Python、Nginx 等。

## nvm

nvm allows you to quickly install and use different versions of node via the command line.

`nvm` 是`node`/`npm`版本管家。它在你的目录下创建了一个隐藏文件夹 `~/.nvm`。你通过 `nvm install` 下载的每一个 `Node` 版本，都分别存放在这个文件夹里独立的目录下。当你执行 `nvm use` 时，它其实是在修改你当前终端窗口的 `PATH` 环境变量。它把指向旧版本的路径删掉，把指向新版本的路径加上去。
...
`npm` 是 `Node.js` 默认的包管理工具，用于安装、发布和管理 `JavaScript` 包。它随同 `Node.js` 一起安装，无需额外安装。`npm` 在功能上比较全面，适用于大多数项目。
...
输入 `npm install` 命令并运行之后，会经历如下几个阶段（以 npm 5.5.1 为例）：
...
首先需要做的是确定工程中的首层依赖，也就是 `dependencies` 和 `devDependencies` 属性中直接指定的模块（假设此时没有添加 `npm instal`l 参数）。
...
上述命令为功能包B在全局创建一个软链接，然后将其链接到项目C模块安装路径 `./node_modules/`，等同于生成了本地模块的符号链接。当你的项目不再需要该模块的时候，需要解除软连接，否则当你在项目中安装 `npm` 上的包时将会出错。
...
`npx` 是 `npm` 的一部分，从 `npm` 5.2.0 开始引入。它用于**临时安装和运行包**，而无需全局安装它们。常用于运行临时脚本，或者在不全局安装 CLI 工具的情况下使用它们。
...
`yarn` 是 Facebook 创建的另一个包管理工具，旨在提供更快速、一致和安全的依赖管理。它有类似 `npm` 的命令，如 `yarn install` 和 `yarn start`。`yarn` 具有锁定依赖版本的功能，以确保在不同环境中的一致性。
...
软连接是一个特殊的文件，其中包含了指向另一个文件或目录的**路径**。它们是**路径的符号引用，而不是实际的文件数据**。
...
在开发过程中选择使用哪个工具取决于你的项目需求和个人偏好。`npm` 是默认和普遍使用的工具，`yarn` 提供了一些额外的优势，而 `pnpm` 则专注于解决依赖冗余问题。 `npx` 则是一个有用的辅助工具，用于在不全局安装包的情况下执行命令。
