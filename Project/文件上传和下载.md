...
在 Web 中，文件上传和下载本质上是浏览器与服务端之间的二进制数据传输问题。
...
const uploadLargeFile = async (file) => {
  ...
  // 2. 并发上传（简单演示，实际建议控制并发数）
  const requestList = chunks.map(({ file }, index) => {
    const formData = new FormData()
    formData.append("chunk", file)
    formData.append("hash", `${file.name}-${index}`) // 简单标识
  ...
  await fetch("/api/merge", {
    ...
    body: JSON.stringify({ filename: file.name, size: CHUNK_SIZE }),
  })
}
```
...
```js
// 断点续传
// 使用 spark-md5 计算计算文件 Hash（可以在 Web Worker 中运行防止卡顿）
...
const saveFile = (data, fileName) => {
  ...
  const url = URL.createObjectURL(blob) // 创建内存指向
...
这种方法适用于需要在前端生成文件内容的情况，比如将字符串、JSON 数据等转换为文件进行下载，前端导出 JSON/CSV 文件等。

- 使用 `Fetch` API：

如果需要从服务器获取文件并进行下载，可以使用 Fetch API 获取文件数据，然后将数据设置为 `Blob` 对象，最后生成下载链接。
...
```js
async function concurrencyDownload(url, concurrency = 3) {
  // 1. 发起 HEAD 请求获取文件总长度
  const headResponse = await fetch(url, { method: "HEAD" })
  const contentLength = parseInt(headResponse.headers.get("Content-Length"))

  if (!contentLength) throw new Error("服务器不支持获取长度或文件为空")

  // 2. 划分为 5MB 一个切片
  const CHUNK_SIZE = 5 * 1024 * 1024
  const chunksCount = Math.ceil(contentLength / CHUNK_SIZE)
  const results = new Array(chunksCount) // 用于按索引顺序存放结果
  ...
  async function worker() {
    while (currentChunk < chunksCount) {
      ...
      const end = index === chunksCount - 1 ? contentLength - 1 : start + CHUNK_SIZE - 1

      console.log(`正在下载切片 ${index}: ${start}-${end}`)

      const res = await fetch(url, {
        headers: { Range: `bytes=${start}-${end}` },
      })

      // 注意：这里拿回的是 ArrayBuffer
  ...
  // 5. 合并并触发下载
  const finalBlob = new Blob(results, { type: headResponse.headers.get("Content-Type") })
  const downloadUrl = URL.createObjectURL(finalBlob)
  const a = document.createElement("a")
  a.href = downloadUrl
  a.download = "big-file-result"
  a.click()
...
async function downloadAndUnzip(url) {
  ...
  const unzip = new Unzip((file) => {
    // 只要解压出一个文件，就会触发这个回调
    console.log(`发现文件: ${file.name}`)

    // 这里可以继续用流式方式处理解压出的单个文件
  ...
  while (true) {
    ...
    if (done) {
      unzip.push(new Uint8Array(0), true) // 告诉解压器结束了
      break
    }
    unzip.push(value) // 把下载的压缩碎片“喂”进去
  }
}
```
...
`createObjectURL` 是 `JavaScript` 中的一个方法，通常用于在网页中创建一个临时的 `URL`，以便在浏览器中显示或处理二进制数据，比如图片、视频、音频等。这个方法通常与 `Blob` 或 `File` 对象一起使用。以下是 `createObjectURL` 方法的基本用法：
...
[File and Blob API in Depth](https://dev.to/omriluz1/file-and-blob-api-in-depth-20lj?context=digest)
