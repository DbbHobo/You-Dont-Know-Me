...
使用场景：`jQuery` 的选择器`$(selector)`、`Vue` 创建 `VNode`、`React.createElement()` 创建 React 元素
...
使用场景：`Vuex` 的 `store` 实例、在 React 的 `Context API` 提供的全局共享状态，其实就是一种单例思想的体现
...
使用场景：React 中高阶组件（HOC）是装饰器模式的一种实现
...
class NewApiAdapter {
  ...
  fetch() {
    return this.oldApi.getData() // 适配旧接口
  }
}
```
