...
`let obj = { name: "xxx" }` 的存储过程如下：

在**栈**中：

- 分配一个变量 `obj`，存放的是一个地址引用（指向堆内存的某一块区域）

在**堆**中：

- 分配一块动态内存空间，存储实际数据：`{ name: "xxx" }`
- 这块内存可能不是连续的，但通过引用地址可以找到它
...
2. 读取里面保存的引用地址；
3. 根据地址到 **堆** 里找到对象；
4. 在对象的属性表中找到 `name` 对应的值 `"xxx"`；

### 两者区别

- **栈**是先进后出的，就像一个桶，后进去的先出来，它下面本来有的东西要等其他出来之后才能出来。栈区（stack） 由编译器自动分配释放 ，存放函数的参数值，局部变量的值等。
- **堆**是在程序运行时，而不是在程序编译时，申请某个大小的内存空间。即动态分配内存，对其访问和对一般内存的访问没有区别。对于堆，我们可以随心所欲的进行增加变量和删除变量，不用遵循次序。堆区（heap） 一般由人工分配释放，若人工不释放，程序结束时可能由 OS 回收。

![数据存储](../assets/storage.jpg)
![数据存储](../assets/the_javascript_runtime_environment_example.svg)
